...
import pickle
from flask import Flask, request, jsonify  # Add Flask imports for webhook
...
# Add a function to get OAuth credentials and cache them
OAUTH_SCOPES = ['https://www.googleapis.com/auth/drive.file']
OAUTH_TOKEN_PICKLE = 'token_drive_oauth.pickle'

def get_oauth_drive_creds():
    creds = None
    if os.path.exists(OAUTH_TOKEN_PICKLE):
        with open(OAUTH_TOKEN_PICKLE, 'rb') as token:
            creds = pickle.load(token)
    if not creds or not creds.valid:
        if creds and creds.expired and creds.refresh_token:
            creds.refresh(Request())
        else:
            flow = InstalledAppFlow.from_client_secrets_file('client_secret.json', OAUTH_SCOPES)
            creds = flow.run_local_server(port=0)
        with open(OAUTH_TOKEN_PICKLE, 'wb') as token:
...
def send_email_with_multiple_pdfs(to_email, message, pdf_files, user_name=None, whatsapp_number=None, email_address=None, birth_date=None, location=None):
    ...
    if last_response_pdf and user_name:
        ...
        if pdf_url:
            ...
            # Create a local file path reference instead
            pdf_url = f"Local file: {os.path.abspath(last_response_pdf)}"
    ...
    if top_match_pdfs and user_name:
        ...
        if top_match_urls:
            ...
            # Create local file path references instead
            top_match_urls = [f"Local file: {os.path.abspath(pdf)}" for pdf in top_match_pdfs]
...
def upload_pdf_to_drive_and_get_url(pdf_filename, user_name):
    ...
    try:
        ...
        print("DETAILED GOOGLE API ERROR (OAuth):", str(e))
        return None
...
# --- Flask Webhook Setup ---
# Webhook URL: https://1cdb17c6dffd.ngrok-free.app/app-webhook
app_flask = Flask(__name__)
WEBHOOK_SECRET = os.getenv("WEBHOOK_SECRET", "m5Z7q_hQtYPiO8VUXB0GSOPw3AC7RuDklwmu4JWpbVY")

@app_flask.route('/app-webhook', methods=['POST'])
def app_webhook():
    auth_header = request.headers.get('Authorization')
    if auth_header != f'Bearer {WEBHOOK_SECRET}':
        logger.warning("Unauthorized webhook request to /app-webhook")
        return jsonify({'error': 'Unauthorized'}), 401
    data = request.get_json()
    logger.info(f"Received webhook data at /app-webhook: {data}")
    # Optionally, you can pass data to your processing function
...
def health():
    return jsonify({'status': 'healthy', 'service': 'app.py', 'timestamp': datetime.now().isoformat()}), 200
...
if __name__ == "__main__":
    try:
        ...
        # Start Flask app for webhook if desired
        import threading
        def run_flask():
            port = int(os.getenv("PORT", 5001))  # Use 5001 to avoid conflict with webhook_server.py
